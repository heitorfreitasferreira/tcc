...
| P39 | Substituir `itemize` por `alineas` em enumerações formais | `abntex2` fornece ambiente `alineas` (itens a), b), c)...). Substituir `itemize` onde o conteúdo for enumeração formal dentro de seção (ex: perguntas de pesquisa em `introducao.tex`, objetivos). Descoberto em [[auditoria-formato-abnt]]. | Concluída |
...
Itens opcionais, baixa prioridade — registrados para caso haja tempo antes da entrega final.

| ID | Descrição | Esforço | Origem |
|----|-----------|:-------:|--------|
| FUT-01 | Complementar eryoldas2022survey (Q2 MDPI) com referência Q1 sobre tuning (ex: López-Ibáñez et al. 2016, *Operations Research Perspectives*, OA via repositório ULB) | Baixo | Council P33 GAP-3 |
| FUT-02 | Adicionar nota de rodapé em experimentos.tex:31 sobre natureza qualitativa do survey (sem benchmark experimental unificado) | Baixo | Council P33 F4 |
| FUT-03 | Obter PDFs dos 3 papers originais de P33 via CAPES/UFU e re-adicionar ao BibTeX como referências complementares | Médio | Council P33 |
| FUT-04 | Baixar PDF dos ~10 papers do vault sem PDF (marcados ❌ no index) via Portal CAPES | Médio | Index vault |
| FUT-05 | Atualizar canvas com papers de tuning adicionais se FUT-03 for executado | Baixo | Canvas |
...
**P39 concluída** — 4 ocorrências de `itemize` substituídas por `alineas` em `introducao.tex` (objetivos, perguntas, contribuições) e `experimentos.tex` (métricas). Council PASS/PASS em `.agents/council/2026-06-05-P39.md`.

**Em seguida** — pendências ainda abertas: **P24** (vincular claims ao texto `.tex`), **P25** (relatório anti-alucinação), **P40** (ficha catalográfica) e **P41** (limpeza de preâmbulo). Escrever os capítulos na ordem: Proposta → Experimentos → Fundamentação → Introdução → Conclusão. Por último, **P11** (Resumo/Abstract).
