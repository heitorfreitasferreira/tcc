...
**Em seguida** — pendências ainda abertas: **P14** (apêndices), **P18** (bibliografia mínima por capítulo), **P24** (vincular claims ao texto `.tex`), **P25** (relatório anti-alucinação), **P33** (literatura de tuning), **P37** (`\ac{}`), **P39** (`alineas`), **P40** (ficha catalográfica) e **P41** (limpeza de preâmbulo). Escrever os capítulos na ordem: Proposta → Experimentos → Fundamentação → Introdução → Conclusão. Por último, **P11** (Resumo/Abstract).
